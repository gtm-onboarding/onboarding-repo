...
export function CheckoutPage() {
  const { items, totalPrice, clearCart, riderPrice } = useCart();
  const { user } = useAuth();
  ...
  const taxableTotal = totalPrice + riderPrice;
          ...
          >
            ...
            <div style={{ borderTop: '1px solid #F0EEEB', marginTop: '20px', paddingTop: '20px' }}>
              ...
              {riderPrice > 0 && (
                ...
                >
                  <span>Jewelry Coverage Rider ({(JEWELRY_RIDER_RATE * 100).toFixed(0)}%)</span>
                  <span style={{ color: '#1A1A1A' }}>${riderPrice.toFixed(2)}</span>
                </div>
              )}
              <div
                style={{
