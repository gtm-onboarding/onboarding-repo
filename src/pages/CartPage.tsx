...
export function CartPage() {
  const { items, totalPrice, clearCart, riderSelected, setRiderSelected, riderEligible, riderPrice } = useCart();
          ...
          {riderEligible && (
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '16px' }}>
              <label style={{ color: '#6B6B6B', fontSize: '15px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                />
                Jewelry Coverage Rider ({(JEWELRY_RIDER_RATE * 100).toFixed(0)}%)
              </label>
